...
function Estoque({ dados, onAdicionar, onRemover, onAtualizar, onAdicionarVariante, onRemoverVariante, onAtualizarVariante }) {
  ...
  const [form, setForm] = useState({ nome: "", descricao: "", precoCompra: "", precoVenda: "", quantidadeEstoque: "", quantidadeMinima: "5", sku: "", imagem: "", categoria: "", console: "", marca: "", condicao: "Novo", codigoBarras: "", fornecedor: "" });
  ...
  function abrirModal(p = null) {
    if (p) { setEditando(p.id); setForm({ nome: p.nome, descricao: p.descricao || "", precoCompra: p.precoCompra, precoVenda: p.precoVenda, quantidadeEstoque: p.quantidadeEstoque, quantidadeMinima: p.quantidadeMinima, sku: p.sku || "", imagem: p.imagem || "", categoria: p.categoria || "", console: p.console || "", marca: p.marca || "", condicao: p.condicao || "Novo", codigoBarras: p.codigoBarras || "", fornecedor: p.fornecedor || "" }); }
    else { setEditando(null); setForm({ nome: "", descricao: "", precoCompra: "", precoVenda: "", quantidadeEstoque: "", quantidadeMinima: "5", sku: "", imagem: "", categoria: "", console: "", marca: "", condicao: "Novo", codigoBarras: "", fornecedor: "" }); }
    setModal(true);
  }
  ...
  function submit(e) {
    ...
    const d = { nome: form.nome, descricao: form.descricao, precoCompra: parseFloat(form.precoCompra) || 0, precoVenda: parseFloat(form.precoVenda), quantidadeEstoque: parseInt(form.quantidadeEstoque) || 0, quantidadeMinima: parseInt(form.quantidadeMinima) || 5, sku: form.sku, imagem: form.imagem || "", categoria: form.categoria || "", console: form.console || "", marca: form.marca || "", condicao: form.condicao || "Novo", codigoBarras: form.codigoBarras || "", fornecedor: form.fornecedor || "" };
      ...
      <div className="card">
        <div className="table-wrap">
          {produtos.length === 0
            ...
            : <table>
              <thead><tr><th>Produto</th><th>Categoria</th><th>Console</th><th>Condição</th><th>SKU</th><th>Compra</th><th>Venda</th><th>Estoque</th><th>Margem</th><th></th></tr></thead>
              <tbody>
                {produtos.map(p => {
                  const vars = variantesProduto.filter(v => v.produtoPaiId === p.id);
                  const temVars = vars.length > 0;
                    ...
                    ...(temVars && expandido ? vars.map(v => {
                      ...
                      return (
                        <tr key={`var-${v.id}`} className="variante-row">
                          ...
                          <td style={{ color: "var(--text2)", fontSize: 11 }}>—</td><td style={{ color: "var(--text2)", fontSize: 11 }}>—</td><td style={{ color: "var(--text2)", fontSize: 11 }}>—</td><td style={{ color: "var(--text2)", fontSize: 12 }}>—</td><td style={{ color: "var(--text2)", fontSize: 12 }}>—</td><td style={{ color: "var(--text2)", fontSize: 12 }}>—</td>
      ...
      <Modal open={modal} onClose={() => setModal(false)} title={editando ? "Editar Produto" : "Novo Produto"} wide>
        <form onSubmit={submit}>
          <div className="form-grid form-grid-2">
            ...
            <div className="input-group"><label className="input-label">Código de Barras</label><input className="input" value={form.codigoBarras} onChange={e => set("codigoBarras", e.target.value)} /></div>
            <div className="input-group">
